...
init-test-db: ## Initialize database for testing
	...
	$(DOCKER_EXEC_DEV) php bin/console doctrine:database:drop --force --env=test --if-exists || true
	$(DOCKER_EXEC_DEV) php bin/console doctrine:database:create --env=test
	$(DOCKER_EXEC_DEV) php bin/console doctrine:migrations:migrate --env=test --no-interaction
...
# Deployment
deploy: start install ## Deploy project
